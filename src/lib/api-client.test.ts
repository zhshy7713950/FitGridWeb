...
async function capturedRequestHeaders(headers: HeadersInit): Promise<Headers> {
  const fetcher = vi.fn().mockResolvedValue(new Response("ok"));
  vi.stubGlobal("fetch", fetcher);

  await requestResponse("/health", { headers });
...
describe("requestResponse", () => {
  ...
  it("preserves record request headers", async () => {
    const headers = await capturedRequestHeaders({ "X-Request-Shape": "record" });

    expect(headers.get("X-Request-Shape")).toBe("record");
    expect(headers.get("Accept")).toBe("application/json");
  });

  it("preserves Headers request headers", async () => {
    const headers = await capturedRequestHeaders(new Headers({ "X-Request-Shape": "headers" }));

    expect(headers.get("X-Request-Shape")).toBe("headers");
    expect(headers.get("Accept")).toBe("application/json");
  });

  it("preserves tuple array request headers", async () => {
    const headers = await capturedRequestHeaders([["X-Request-Shape", "tuples"]]);

    expect(headers.get("X-Request-Shape")).toBe("tuples");
    expect(headers.get("Accept")).toBe("application/json");
  });

  it("respects an explicitly supplied Accept header regardless of casing", async () => {
    const headers = await capturedRequestHeaders({ accept: "application/octet-stream" });
