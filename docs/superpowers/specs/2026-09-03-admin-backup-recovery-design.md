...
管理员授权威胁模型假设应用进程未被攻陷；攻击者已取得 app-process RCE 不在该模型内，因此不应把本设计表述为能阻止已攻陷应用发起所有允许的维护动作。root worker 仍只执行仓库内固定路径的固定命令；便携备份的上传数据仅按固定表和固定字段解码，不作为 shell、SQL、DDL 或 `pg_restore` 输入执行。
